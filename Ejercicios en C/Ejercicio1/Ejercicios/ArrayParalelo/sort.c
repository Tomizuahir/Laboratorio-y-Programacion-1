...
int burbujeoMejoradoInt(char apellido[][20],int nota1[5],int nota2[5],float promedio[5],int tamArrayNums,int ascend)
{
    ...
    do
    {
        ...
        for(i=0;i<tamArrayNums-1;i++)
        {
            if((ascend==1 && apellido[i]>apellido[i+1]) || (ascend==0 && apellido[i]<apellido[i+1]))
            {
                auxSwap=apellido[i];
                apellido[i]=apellido[i+1];
                apellido[i+1]=auxSwap;
